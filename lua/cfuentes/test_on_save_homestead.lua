local ns_name = 'php-auto-test-homestead'
local ns = vim.api.nvim_create_namespace(ns_name)
local group = vim.api.nvim_create_augroup('php-automagic-homestead', { clear = true })
...
local find_test_line = function(bufnr, entry)
    ...
    vim.api.nvim_buf_call(bufnr, function()
        result = vim.fn.search(entry.methodName, "n")
    end)
    return result - 1
end
...
local attach_to_buffer = function(bufnr, command, folder)
    local state = {
        bufnr = bufnr,
        tests = {},
    }
    ...
    vim.api.nvim_create_autocmd('BufWritePost', {
        ...
        callback = function()
            ...
            local after_test = function()
                ...
                local name = string.gsub(folder, "(.*/)(.*)", "%2")
                local cat_command = { "xq", "-c", ".", "/tmp/" .. name }
...
vim.api.nvim_create_user_command("PhpTestOnSaveHomestead", function()
    local buffer = vim.api.nvim_get_current_buf()
    local file = vim.api.nvim_buf_get_name(buffer)
    local folder = vim.api.nvim_command_output([[pwd]])
    local filename = string.gsub(file, "(.*/)(.*)", "%2"):gsub('.php', '')
    attach_to_buffer(buffer, { "homestead_test_xml", folder, "--filter=" .. filename }, folder)
end, {})
